...
3. **No public docs page yet.** Only `docs/fuse/FUSE.md`, internal. Fuse stays behind `FUSE_ENABLED` (off) and the token-gated `/fuse` harness until the implementation, vectors, verifier behaviour and claim language are fully tested; public documentation comes later under the spec's rollout (section 17).

## Deploy checklist (only on Mike's word; nothing deployed as of 2026-09-03)

Order: verify 1.4.0, audit 0.4.0, player 0.8.0 (Mike runs the OTP publishes); core `@mikeargento/bitgraph` (the site depends on it by `file:..`, so the Vercel build needs the core package built or a published version); the parent patch on the host (the files in `git diff --stat main -- server/commit-service/src`, then `tsc -p tsconfig.parent.json` and a restart, which is seconds of 503; `FUSE_ENABLED` unset keeps held slots off); site env (`FUSE_ENABLED`, `FUSE_HARNESS_ENABLED`, `FUSE_HARNESS_TOKEN`, all unset by default); extend the Vercel WAF rule `commit-rate-limit` (today POST `/api/commit` only) to POST `/api/fuse/allocate` and `/api/fuse/commit`. The Imran freeze governs the host and the published packages until it lifts.
