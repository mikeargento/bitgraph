...
Deliverable 6, ledger indexing: `website/src/lib/s3.ts` (origin index write when `attribution.name === "BitGraph Fuse"` and `message` decodes to 32 bytes; `getProofsByDigest` returns `kind`), `api/proofs/digest/[digest]/route.ts` (positions carry `kind` and `artifactDigest`), `app/proof/[digest]/page.tsx` (labels, descendants unranked), `api/verify/route.ts`, `app/mcp/route.ts`, `packages/zapier` (fused-only is not on record for the origin bytes), camera solo path.

Deliverable 7, SDK: new workspace `packages/fuse/` (proprietary, like the core package; verification stays MIT): `fuse(builder, options)` allocating through `/api/fuse/allocate`, handing the commitment and origin digest to the builder, hashing, committing through `/api/fuse/commit` under the same slot, returning a Frame; the recovery rule from Q3; nonce hygiene per spec 9.4.
