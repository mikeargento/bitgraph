...
## Ruling 8 (2026-09-03): the hosted endpoint makes BitGraphs the default way
...
## Ruling 9 (2026-09-03, evening): the proof page reads as one page, and the export is a package

Mike, on the proof page after Fuse became the default: "is this the most intuitive layout?" then, on a mockup that regrouped the page into labelled per-file cards, "that looks messy", and decisively: "nothing has to 'say' what a fused proof is. i used the word intuitive and maybe you took that wrong. im referring to field placement UX/UI". So no explanatory prose anywhere on the page. What changed is placement:

- The two hashes moved out of the receipt into a collapsed **Hashes** card directly under it (new file first, then the original, per his earlier ruling). An ordinary recording keeps its inline File Hash.
- **No Submitter's Note card on a fused proof.** The attribution is the signed marker, not a person's note; it stays in Raw JSON.
- **One proof, one page.** "there should be no separate lookup. original and new find same proof": the Positions card speaks in one voice for every position ("N positions. Each sits at its own place in the sequence…"), a descendant row reads "New file made from these bytes", the home drop no longer tags a match "(fused · placement)", and both MCP `check` tools report `on_record` for either hash with no separate descendant count. "Original" is reserved for the file a new file was made from; a repeated recording's earliest row says "earliest".
- The finder box on the proof page is no longer the home frame's 3:2. It is content-sized, `min-height: clamp(200px, 30vw, 236px)`: "finder height could be shorter i suppose. but verbiage needs room".
- **The export is the package**, one button named "Export BitGraph package" on the proof page and the results card. It carries the original, `proof.json`, the Frame, the new file under `fused/`, and `ethereum-anchors/`. The separate "Download new file" link is gone. Mike's reasoning: "because intuitively that is when a user will want access to that new file right?" The new file stays virtual everywhere else and is built at export time, from the original in hand (`rebuildFromOrigin`) or taken back out of the new file in hand (`unpackNewFile`, new in `fuse-client.ts`). Verified against the production artifact at 9792: A rebuilds the fused bytes exactly, B recovers the original exactly, a tampered new file yields NO_MATCH and no original. The player reads the package folder as-is (TRUE on `fused/preston.fused.jpg`).

Also swept this round: the public verb is **make**, never take/taking/taken (npm MCP package bumped to 0.2.1 for its share of the text; publish is Mike's call).
