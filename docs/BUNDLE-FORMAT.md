...
An anchor proof by itself proves causal position: everything before it in the chain existed before the anchor commit that consumed the block hash. It does not by itself prove wall-clock time, because the block timestamp is not in the signed body. The anchor witness exists to add wall-clock evidence offline: it carries the raw Ethereum block header so a consumer can locally recompute the block hash and, only after that recomputation succeeds, read the timestamp out of the header itself.
...
6. **Only now, read the timestamp.** Decode the header's `timestamp` field (the 12th RLP item, index 11, big-endian unsigned integer, Unix seconds). These indices are stable across forks; fork-dependent fields are appended after them. This timestamp is external wall-clock evidence for the anchor's position in the chain, in two directions of different strength. Proofs causally AFTER the anchor were committed no earlier than this timestamp: the block hash was unpredictable before it and the anchor commit consumed it. Proofs causally BEFORE the anchor existed before the anchor commit, which is proven to have come at or after this timestamp, not how promptly; reading the timestamp as a wall-clock ceiling for them additionally assumes the anchor consumed a recently published block. Consumers MUST state that assumption wherever they report the ceiling. The timestamp bounds segments; it does not date individual proofs.
