...
BitGraph records a file's SHA-256 digest at a causal position in a forward-only ledger, anchored to Ethereum. This server hashes files locally; only the digest ever leaves the machine. File contents are never uploaded.
...
- `bitgraph_get_proof` fetches a proof by digest, BitGraph number, or file path, including the two-sided Ethereum anchor window that brackets when it was BitGraphed. Read-only.
