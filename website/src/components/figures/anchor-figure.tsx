...
export function AnchorFigure() {
  ...
  const wide = (
    <Svg w={960} h={268} title="Three Ethereum blocks drawn above a sequence of positions. Each block's hash is committed as an anchor on the sequence, slightly later than the block. A record sits between the first and second anchors. Brackets mark the floor, from the first anchor's block to the record, and the ceiling, from the record to the second anchor.">
      ...
      {[[150, "block 25,984,342", "17:25:47 UTC"], [470, "block 25,984,343", "17:25:59 UTC"], [790, "block 25,984,344", "17:26:11 UTC"]].map(([x, b, t]) => (
        <g key={String(x)}>
          <rect className="box-b" x={Number(x) - 90} y={36} width={180} height={52} />
      ...
      <Arrow d="M 150 90 L 190 172" tone="b" />
      <Arrow d="M 470 90 L 640 172" tone="p" />
      <Arrow d="M 790 90 L 900 172" tone="b" />
      <T x={230} y={140} cls="lbl" size={11}>the block hash, committed as a file</T>

      <T x={0} y={164} cls="ttl">THE BITGRAPH SEQUENCE</T>
      <line className="rule" x1={0} y1={y} x2={960} y2={y} />
      <Others y={y} xs={[40, 82, 124, 260, 302, 344, 500, 542, 584, 720, 762, 804, 846]} />
      ...
      <Pos cx={900} cy={y} kind="anchor" size={20} />
      ...
      <Chip cx={900} cy={y - 30} text="anchor #4,230" tone="b" />
  ...
  const narrow = (
    <Svg w={360} h={600} title="A vertical sequence of positions with Ethereum blocks drawn to the right. Each block's hash is committed as an anchor slightly later. A record sits between the first and second anchors, with floor and ceiling brackets.">
      ...
      {[[120, "25,984,342", "17:25:47"], [316, "25,984,343", "17:25:59"], [540, "25,984,344", "17:26:11"]].map(([yy, b, t]) => (
        <g key={String(yy)}>
          <rect className="box-b" x={228} y={Number(yy) - 44} width={128} height={46} />
          <text className="num" x={292} y={Number(yy) - 25} textAnchor="middle" style={{ fill: "var(--accent)", fontSize: 11 }}>block {b}</text>
          <text className="lbl" x={292} y={Number(yy) - 9} textAnchor="middle" style={{ fontSize: 10.5 }}>mined {t} UTC</text>
        </g>
      ))}
      <OthersV x={x} ys={[50, 80]} />
      <Pos cx={x} cy={124} kind="anchor" size={20} />
      <Chip cx={x + 74} cy={124} text="anchor #4,190" tone="b" />
      <Arrow d="M 228 100 L 170 120" tone="b" />
