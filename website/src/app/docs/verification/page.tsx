...
export default function VerificationPage() {
  return (
    <article className="prose-doc">
      ...
      <p className="text-[color:var(--text)] mb-6">
        Input: a proof (<code className="text-xs font-mono bg-[color:var(--code-bg)] text-[color:var(--accent)] px-1">BitGraphProof</code>), the artifact bytes (<code className="text-xs font-mono bg-[color:var(--code-bg)] text-[color:var(--accent)] px-1">Uint8Array</code>), and an optional verification policy.
      </p>
        ...
        {[
          ...
          <div key={item.step}>
            <h3 className="text-base font-semibold mb-2">
              <span className="text-[color:var(--dim)] mr-3 tabular-nums">{item.step}.</span>
              {item.title}
            </h3>
            <p className="text-base text-[color:var(--text)] leading-relaxed">{renderInline(item.desc)}</p>
          </div>
        ))}
      </div>

      <p className="text-base text-[color:var(--text)] leading-relaxed mt-6">
        Step 6 confirms PCR0 matches the measurement the proof claims, and the certificate chain proves that measurement came from genuine Nitro hardware. To confirm the measurement itself corresponds to the published enclave source, the build is bit-for-bit reproducible: rebuild it and re-derive the exact PCR0 yourself. See <a href="/docs/self-host-tee" className="text-[color:var(--accent)] font-medium no-underline">reproducible builds</a>.
      </p>

      <h2 className="text-xl font-semibold mt-12 mb-4">Fused artifacts</h2>
      <p className="text-[color:var(--text)] mb-4">
        A fused artifact carries a commitment to its own slot record inside its bytes, written before the artifact was finished. Its proof is an ordinary <code className="text-xs font-mono bg-[color:var(--code-bg)] text-[color:var(--accent)] px-1">bitgraph/1</code> proof whose signed <code className="text-xs font-mono bg-[color:var(--code-bg)] text-[color:var(--accent)] px-1">attribution</code> names the placement and the origin, so the seven steps above run unchanged. <code className="text-xs font-mono bg-[color:var(--code-bg)] text-[color:var(--accent)] px-1">verifyFuse({"{ proof, bytes, frame? }"})</code> in <code className="text-xs font-mono bg-[color:var(--code-bg)] text-[color:var(--accent)] px-1">@mikeargento/bitgraph-verify</code> then adds one comparison, chosen by what the bytes hash to. The commitment and the registered placements are defined in <a href="/docs/proof-format" className="text-[color:var(--accent)] font-medium no-underline">Proof Format</a>.
      </p>
      <div className="overflow-x-auto mb-6">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[color:var(--line)]">
              <th className="text-left py-2 pr-4 text-xs font-medium uppercase tracking-wider text-[color:var(--dim)]">Bytes hash to</th>
              <th className="text-left py-2 pr-4 text-xs font-medium uppercase tracking-wider text-[color:var(--dim)]">Check</th>
              <th className="text-left py-2 text-xs font-medium uppercase tracking-wider text-[color:var(--dim)]">Result</th>
            </tr>
          </thead>
      ...
      <p className="text-[color:var(--text)] mb-4">The statements a verifier may print, verbatim:</p>
      <ul className="space-y-2 mb-6 text-sm text-[color:var(--text)]">
      ...
      <p className="text-base text-[color:var(--text)] leading-relaxed mb-4">
      ...
      <div className="code-block mb-6">
        ...
        <pre className="text-[color:var(--text)]">{`interface VerificationPolicy {
...
`}{`  allowedMeasurements?: string[];     // exact match
  ...
  requireActor?: boolean;             // legacy
      ...
      <div className="space-y-3 mb-8">
        <div className="border-l-2 border-l-[color:var(--line)] pl-4 py-1">
          <code className="text-xs font-mono text-[color:var(--warn)]">requireEnforcement</code>
          <span className="text-sm text-[color:var(--text)] ml-2">alone - prevents in-transit downgrade only</span>
        </div>
        <div className="border-l-2 border-l-[color:var(--line)] pl-4 py-1">
          <code className="text-xs font-mono text-[color:var(--accent)]">requireEnforcement + allowedMeasurements</code>
          <span className="text-sm text-[color:var(--text)] ml-2">- pins to specific enclave image</span>
        </div>
        <div className="border-l-2 border-l-[color:var(--line)] pl-4 py-1">
          <code className="text-xs font-mono text-[color:var(--ok)]">+ requireAttestation</code>
          <span className="text-sm text-[color:var(--text)] ml-2">- full trust (vendor-attested hardware boundary)</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[color:var(--line)]">
              <th className="text-left py-2 pr-4 text-xs font-medium uppercase tracking-wider text-[color:var(--dim)]">Item</th>
              <th className="text-left py-2 text-xs font-medium uppercase tracking-wider text-[color:var(--dim)]">Why</th>
            </tr>
          </thead>
          <tbody className="text-[color:var(--text)]">
            <tr className="border-b border-[color:var(--line)]">
