  ...
  return (
    <article className="prose-doc">
      ...
      <div className="code-block">
        ...
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`npx -p @mikeargento/bitgraph bitgraph-fuse fuse photo.jpg --placement trailer/1 --out ./out
...
# exit 0 fused or verified, 1 refused or contradicted, 2 undetermined, 64 usage`}</pre>
      ...
      <div className="code-block">
        ...
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`import { fuse, builderFor } from "@mikeargento/bitgraph";
...
// to place the commitment inside a format you produce yourself.`}</pre>
      ...
      <div className="code-block">
        <div className="code-block-header"><span>Shell</span><CopyCode /></div>
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`# 1. Hash your file
...
curl -X POST https://bitgraph.ing/api/commit \\
  ...
  -d '{
    ...
    "metadata": {
      ...
      <div className="code-block">
        <div className="code-block-header"><span>TypeScript</span><CopyCode /></div>
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`// Hash locally
...
console.log(proof.attribution);      // signed creator metadata`}</pre>
      ...
      <div className="code-block">
        <div className="code-block-header"><span>TypeScript</span><CopyCode /></div>
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`const resp = await fetch("https://bitgraph.ing/api/commit", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    digests: [
...
// proofs[0], proofs[1], proofs[2] - one per digest`}</pre>
      ...
      <div className="code-block">
        <div className="code-block-header"><span>TypeScript</span><CopyCode /></div>
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`import { verify } from "@mikeargento/bitgraph-verify";`}{`
...
if (result.valid) {
  ...
  console.error("Verification failed:", result.reason);
      ...
      <p className="text-[color:var(--text)] mb-4 mt-6">
      ...
      <div className="code-block">
        <div className="code-block-header"><span>TypeScript</span><CopyCode /></div>
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`import { verifyFuse } from "@mikeargento/bitgraph-verify";
...
result.statements;    // the bounded statements, verbatim`}</pre>
      ...
      <div className="code-block">
        <div className="code-block-header"><span>Shell</span><CopyCode /></div>
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`DIGEST=$(openssl dgst -sha256 -binary myfile.pdf | base64)
...
# }`}</pre>
      ...
      <div className="code-block">
        <div className="code-block-header"><span>Shell</span><CopyCode /></div>
        <pre className="text-xs font-mono leading-relaxed text-[color:var(--text)] overflow-x-auto">{`# Get enclave public key and measurement
