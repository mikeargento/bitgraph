      ...
      <article className="prose">
        ...
        <div className="callout is-limit">
          ...
          {/* Mike, 2026-09-18: "there is no ledger" in the protocol; the site keeps copies "just to make the site work better"
              ...
              confirmed while the site answers lookups. The copy is PUBLIC: /ledger lists kept proofs by day
              with their digests (lib/ledger-feed rows carry `digest`). "Fingerprint, never its contents", not "only X, Y, Z": optional
              fields such as a submitter's note can ride along. */}
          <p>A proof holds a record&rsquo;s fingerprint, never its contents, and the protocol has no ledger. BitGraph currently keeps a public copy of each proof: this site lists them by day and looks them up by fingerprint. The copy is a convenience for the site, separate from the protocol, and could stop at any time without affecting any proof. It carries two exposures. Copies already kept cannot be deleted for ten years, by anyone, including BitGraph. And while the site shows them, anyone who can guess a record&rsquo;s exact bytes can confirm it was recorded; that matters for low-entropy records, such as a short form from a known template. The record itself never leaves your systems.</p>
