  ...
  return (
    <div style={{ width: "90%", maxWidth: 800, margin: "0 auto", padding: "40px 0 80px" }}>
      ...
      <p style={{ fontSize: 16, lineHeight: 1.75, color: "#1f2937", margin: "0 0 40px" }}>
        {/* ⚠️ This pointed at "a copy is indistinguishable from the original",
            which the example no longer demonstrates: the three images were each
            GENERATED by a different model from the one before it, so they share
            no bytes and none of them is a copy. It now points at the claim two
            paragraphs up, that a hash cannot say which file existed first,
            which is what the example actually proves. Keep the two in step. */}
        That the order has to come from somewhere else is not a claim you have
        to take on faith. Three AI images, each generated from the one before
        it, signed by three different companies, and not one of those signatures
