...
export default function Harness() {
  ...
  return (
    <main style={{ maxWidth: 800, margin: "0 auto", padding: "40px 20px", color: "var(--ink)", fontSize: 16, lineHeight: 1.5 }}>
      ...
      <form onSubmit={run} style={{ display: "grid", gap: 16, marginBottom: 32 }}>
        ...
        <label style={{ display: "grid", gap: 6 }}>
          <span>Placement</span>
          <select value={placement} onChange={(e) => setPlacement(e.target.value as Placement)} style={{ padding: 8, borderRadius: 0, border: "1px solid var(--line)", background: "var(--panel)", maxWidth: 320 }}>
        ...
        <label style={{ display: "grid", gap: 6 }}>
          ...
          <input type="password" value={token} onChange={(e) => setToken(e.target.value)} autoComplete="off" style={{ padding: 8, borderRadius: 0, border: "1px solid var(--line)", maxWidth: 320 }} />
        </label>
        <div>
          <button type="submit" disabled={!file || busy} style={{ padding: "10px 16px", borderRadius: 0, border: "1px solid var(--accent)", background: "var(--panel)", color: "var(--accent)", fontWeight: 600, cursor: busy ? "wait" : "pointer" }}>
      ...
      {error && (
        <p style={{ border: "1px solid var(--line)", background: "var(--panel)", padding: 16, margin: "0 0 24px" }}>{error}</p>
      )}

      {result && (
        <section style={{ border: "1px solid var(--line)", background: "var(--panel)", padding: 16, display: "grid", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 24, flexWrap: "wrap", fontSize: 14 }}>
            ...
            {fusedHref && <a href={fusedHref} download={result.fusedName} style={{ color: "var(--accent)" }}>Fused bytes: {result.fusedName}</a>}
          </div>
        </section>
      )}
    </main>
